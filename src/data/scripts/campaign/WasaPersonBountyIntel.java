...
public class WasaPersonBountyIntel extends BaseIntelPlugin implements EveryFrameScript, FleetEventListener {
	public static Logger log = Global.getLogger(WasaPersonBountyIntel.class);
...
//		}
		...
		for (EveryFrameScript s : WasaPersonBountyManager.getInstance().getActive()) {
			WasaPersonBountyIntel bounty = (WasaPersonBountyIntel) s;
...
//	}
	...
	private void spawnFleet() {
		...
		if (bountyType == BountyType.REMNANT) {
			fleetFactionId = Factions.REMNANTS;
		}
		if (bountyType == BountyType.MERC) {
			fleetFactionId = Factions.MERCENARY;
		}
		if (bountyType == BountyType.DERELICT) {
